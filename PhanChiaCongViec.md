...
| 1.2      | `feature/config-constants`  | Toàn bộ file /config (app, constants, status, messages, business_rules, **roles.php mới**)                                                                             | `config/*.php`                                                         | Nhân     | PHP constants          | 🔴      | ✅ "Đã xong (BE)"     |
| 1.3      | `feature/config-lang`       | File đa ngôn ngữ vi/en                                                                                                                                                 | `config/lang_vi.php`, `lang_en.php`                                    | Nhân     | PHP array, Session     | 🔴      | ✅ "Đã xong (BE)"     |
...
| #       | Branch                              | Task                                                                                   | File đầu ra                             | Người    | Kỹ thuật                 | Ưu tiên | Trạng thái         |
| ------- | ----------------------------------- | -------------------------------------------------------------------------------------- | --------------------------------------- | -------- | ------------------------ | ------- | ------------------ |
| **7.1** | **`feature/tenant-dashboard`**      | **Dashboard KH: tóm tắt HĐ, ngày hết hạn, số tiền nợ hiện tại**                        | **`modules/tenant/dashboard.php`**      | **Nhật** | **PDO, Bootstrap**       | 🔴      | ✅ "Đã xong (Full)"  |
| **7.2** | **`feature/tenant-hopdong`**        | **Chi tiết HĐ + trạng thái từng phòng**                                                | **`modules/tenant/hop_dong.php`**       | **Nhật** | **PDO JOIN**             | 🔴      | ✅ "Đã xong (Full)"  |
...
| **8.3** | **`feature/global-search`**      | **Tìm kiếm toàn cục dùng FULLTEXT INDEX thay vì LIKE**                            | **`tim_kiem.php`**                        | **Nhật/Nhân** | **MATCH AGAINST, UNION**  | 🔴      | ✅ "Đã xong (Full)" |
| 8.4     | `feature/notification-center`    | Chuông dropdown AJAX                                                              | `includes/admin/notifications.php`        | Nhật     | AJAX, PDO                 | 🔴      | ⏳ "Chờ Nhật (FE)"    |
| **8.5** | **`feature/email-notification`** | **Trigger gửi email: hóa đơn mới → KH, HĐ sắp hết hạn → QLN, YC được duyệt → KH** | **`modules/thong_bao/email_trigger.php`** | **Nhân** | **mailer.php, cron-like** | 🔴      | ✅ "Đã xong (BE)" |
...
| **9.2** | **`feature/qr-code-secure`**  | **QR Code dùng JWT token 15 phút (không link public), tenant_portal verify token**                   | **`modules/tenant_portal/index.php`**, **`assets/js/qrcode-init.js`** | **Nhật/Nhân** | **JWT (php-jwt), qrcode.js** | 🔴      | ✅ "Đã xong (Full)" |
| 9.3     | `feature/maintenance-form`    | Form gửi YC sửa chữa + **chọn priority**                                                             | `modules/maintenance/yc_them.php`                                     | Nhân     | PDO, SELECT                  | 🔴      | ✅ "Đã xong (BE)"     |
| **9.4** | **`feature/maintenance-sla`** | **Admin duyệt + tự tính sla_deadline từ CAU_HINH + countdown timer + gửi email khi status đổi**      | **`modules/maintenance/yc_quan_ly.php`**, **`yc_notify.php`**         | **Nhân** | **DateTime, mailer**         | 🔴      | ✅ "Đã xong (BE)"     |
...
| 9.7     | `feature/cau-hinh`            | Admin cấu hình hệ thống (bao gồm LOCK_PHONG_PHUT, SLA, LOCKOUT)                                      | `modules/cau_hinh/index.php`                                          | Nhân     | PDO GET/UPDATE               | 🔴      | ✅ "Đã xong (BE)"     |
| **9.8** | **`feature/datatables`**      | **DataTables trên mọi bảng**                                                                         | **`datatables-init.js`, `header.php`, `footer.php`**                  | **Nhân** | **jQuery Plugin**            | 🔴      | ✅ "Đã xong (Full)"  |
